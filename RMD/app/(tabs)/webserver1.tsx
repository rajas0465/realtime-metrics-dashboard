...
const ServerMetricsScreen: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:3000/metrics/overview1');
        const infoResponse = await axios.get('http://localhost:3000/webserver1/info'); // Fetching server info
