...
const ServerMetricsScreen: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:3000/metrics/overview2');
        const infoResponse = await axios.get('http://localhost:3000/webserver2/info'); // Fetching server info
